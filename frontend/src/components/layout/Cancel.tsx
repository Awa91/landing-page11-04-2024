import React,{useState, useCallback} from "react";

export   const Cancel = React.memo(() => {
    const strokeColor: string = "#87CEEB";
    const strokeWidth: string = "1.5";
    const strokeSize: string = "size-6";
    const [isMobileDevice, setMobileMenu] = useState<boolean>(false);
    const hideMobileMenu = () => {
        setMobileMenu(false);
    };
    const memoizedHideMobileMenu = useCallback(hideMobileMenu, [isMobileDevice])
  

    return (<>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={strokeWidth} stroke={strokeColor} className={strokeSize} onClick={memoizedHideMobileMenu}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                            </svg>
    </>);
  
  });
  